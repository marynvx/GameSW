...
class gameSW(arcade.Window):
    ...
    def setup(self):
        ...
        '''for i in range(1, 3):
            self.enemy_image_right.append(arcade.load_texture(f"Pics/en_fire{i}.png"))
        for i in range(2, 0, -1):
            self.enemy_image_left.append(arcade.load_texture(f"Pics/en_fire{i}.png", flipped_horizontally=True))'''

        '''enemy_coordinate = [[128,92]]
        for coordinate in enemy_coordinate:
            enemy = arcade.Sprite(image_enemy, ENEMY_SCALING)
